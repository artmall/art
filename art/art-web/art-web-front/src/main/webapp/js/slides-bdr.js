...
jQuery.fn.CRselectBox = jQuery.fn.sBox = function(options){
	...
	var aHtml = '<a  href="#"><span class="CRselectValue">'+selectedOptionTxt+'</span></a>';
	$(aHtml).appendTo($wrapHtml);
	var ulHtml = '<ol class="CRselectBoxOptions"></ol>';
	var $ulHtml = $(ulHtml).appendTo($wrapHtml);
	var liHtml = "";
	_self.find("option").each(function(){
		if($(this).attr("selected")){
			liHtml += '<li class="CRselectBoxItem"><a href="#" class="selected"><span rel="'+$(this).attr("value")+'">'+$(this).text()+'</span></a></li>';
		}else{
			liHtml += '<li class="CRselectBoxItem"><a href="#"><span rel="'+$(this).attr("value")+'">'+$(this).text()+'</span></a></li>';
		}
	});
